...
function readRaw(): any[] {
  try {
    ...
    return Array.isArray(parsed?.docs)
      ? parsed.docs
...
export const saveDocsCompat = (docs: DocumentItem[]) =>
  writeRaw(docs.map(normalize));
...
export function docsForAssetCompat(assetKey: string): DocumentItem[] {
  ...
  return loadDocsCompat().docs.filter(
    (d) =>
      (Array.isArray(d.assetIds) && d.assetIds.includes(key)) ||
      (Array.isArray((d as any).assetNumbers) &&
        (d as any).assetNumbers.includes(key)),
  );
}
