...
export function linkDocToAsset(docId: string, assetIdOrNumber: string) {
  ...
  const found = assets.find(
    (a) => a.assetNumber === assetIdOrNumber || a.id === assetIdOrNumber,
  );
  ...
  const nums = new Set(
    Array.isArray((d as any).assetNumbers) ? (d as any).assetNumbers : [],
  );
  nums.add(assetNum);
...
export function unlinkDocFromAsset(docId: string, assetIdOrNumber: string) {
  ...
  (d as any).assetIds = (d.assetIds ?? []).filter(
    (x: string) => x !== assetId && x !== assetNum,
  );
  if (Array.isArray((d as any).assetNumbers)) {
    (d as any).assetNumbers = (d as any).assetNumbers.filter(
      (n: string) => n !== assetNum && n !== assetId,
    );
  }
  d.updatedAt = new Date().toISOString();
  saveDocs({ docs: raw });
...
export const linkDocToAssetNumber = (docId: string, n: string) =>
  linkDocToAsset(docId, n);
export const unlinkDocFromAssetNumber = (docId: string, n: string) =>
  unlinkDocFromAsset(docId, n);
